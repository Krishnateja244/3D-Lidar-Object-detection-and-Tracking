...
def read_calib(calib_path):
    ...
    Args:
        calib_path: Calibration Text File Path

    Returns:
        P2: 3D camera coordinates to 2D image pixels
        vtc_mat: 3D Lidar coordinates to 3D camera coordinates 
    """
...
def read_velodyne(path, P, vtc_mat, IfReduce=True):
    ...
    Args:
        path:  Path to lidar bin file
        P: Camera 3D to Camera 2D Matrix
        vtc_mat:  Lidar 3D to Camera 3D Matrix
        IfReduce: Defaults to True.

    Returns:
        lidar_copy: Valid points in Lidar Coordinates
    """
    max_row = 374  # y
    max_col = 1241  # x
...
def velo_to_cam(cloud,vtc_mat):
    ...
    Args:
        cloud (_type_): 3D Camera Points
        vtc_mat (_type_): Lidar 3D to Camera 3D Matrix
...
def read_detection_label(path):
    ...
    Args:
        path (_type_): Input label path file

    Returns:
        Array of Box coordinates and Array of label names
    """
...
def read_tracking_label(path):
    ...
    Returns:
        Frame Dictionary and Frame Name dictionary
    """
